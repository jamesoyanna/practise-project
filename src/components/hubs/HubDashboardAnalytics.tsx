...
import type { HubDashboardAnalytics } from '../../store/slices/hubdashboard'
...
const StatisticCard = ({ data = {} }: { data: Partial<HubDashboardAnalytics>; }) => {
    let bgColor = '';
    let chevronColor = '';
  // Determine background color and chevron color based on data key
    ...
    return (
        ...
        <div className="relative flex flex-col items-start gap-4">
            ...
            <div className="text-2xl font-extrabold">{data.id && (data.id === 5 || data.id === 6) ? `NGN ${data.value}` : data.value}</div>
...
const Statistic = ({ data = [] }: { data?: Partial<HubDashboardAnalytics>[] }) => {
    return (
