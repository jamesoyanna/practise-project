...
type SmartDeviceDashboardAnalytics = {
    key: string
    value: number
    label: string
...
type StatisticCardProps = {
    data: Partial<SmartDeviceDashboardAnalytics>
}
...
const OnboardedCylinderCard: React.FC<{
    ...
    const onboardedSmartDeviceData = data.find(
        (card) => card.key === 'totalsmartdevice'
    )

    if (!onboardedSmartDeviceData) {
        return null
    }

    return (
        <div className="grid grid-cols-2">
            <StatisticCard data={onboardedSmartDeviceData} />
        </div>
    )
}
